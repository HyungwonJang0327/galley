...
**벨로그 본문**: 입력 = 주제 + EvidenceBundle(요약 + 실제 조각) + 어투 프롬프트. **리포 전체·분석 글 원문은 넣지 않는다.** 구현(2026-09-22 BS2): `VelogInput { topic, bundle, tone, instruction? }` — 이 타입 밖의 것은 프롬프트에 들어갈 수 없다. "요약"은 번들의 `analyses`(항목이 담긴 글의 제목·요약, 인덱싱 때 redact됨). 어투는 system 프롬프트(고정 머리 "근거에 있는 것만·[확인 필요] 주석·대괄호 치환 유지" + 파일 내용), 주제·지시·요약·조각은 user 프롬프트. **근거 조각이 0개면 본문을 쓰지 않는다**(`VELOG_EVIDENCE_EMPTY`, 재시도 불가 — 실행 Dialog의 "0건 경고(실행은 가능)"는 유지하되, 실행하면 근거 수집까지 돌고 본문에서 멈춘다. 요약만으로 본문을 쓰지 않는다는 CLAUDE.md §5 규칙이 우선).
...
**링크드인**(구현 2026-09-23 BS3): 입력 = **벨로그 본문(velog.md 전체) + 어투 프롬프트 + 수정 지시** — `LinkedinInput { topic, body, tone, instruction? }`. EvidenceBundle을 다시 넣지 않는다(본문에서만 파생 — 근거 검증을 본문 직후에 두는 이유와 같은 축). 본문은 ArtifactStore에서 `sources.velog ?? runId`로 읽고 조각과 같은 동적 펜스로 감싼다. 고정 머리는 "사실은 본문에 있는 것만·본문의 [확인 필요] 주석은 확정 사실로 쓰지 않음·식별자는 이미 일반화됐으니 그대로·새 코드 없음·대괄호 토큰 유지". 벨로그 링크 자리 `[벨로그 링크]`는 어투 파일이 정하고 코드는 검사하지 않는다(어투를 고치면 코드를 안 고쳐도 되게). 본문이 없거나 비면 실패(`LINKEDIN_BODY_MISSING`·`LINKEDIN_BODY_EMPTY`, 재시도 불가 — 본문 단계부터 재실행). 출력 상한 `linkedinMaxOutputTokens` 2,048(1,000~1,300자), 잘리면 실패. 산출물 `linkedin.md`.
...
- 2026-09-22 BE8 근거 수집 단계 1 구현에서 정한 것: ① **번들 파일 2곳의 경계는 타입으로** — `EvidenceItem`(snippet 있음, DATA_DIR)과 `EvidencePointerItem`(snippet 키 없음, posts)이 다른 타입이고 `stripSnippets`만이 사본을 만든다. `parseEvidenceBundle`은 snippet 없는 파일을 거부한다(뒤 단계가 포인터 사본을 원본으로 착각하지 않게) ② `EvidenceStore`는 DATA_DIR 전용 인터페이스(`write`·`read`·`remove`, 원자적 쓰기, 슬러그·runId의 경로 구분자는 파일 이름으로만) — blog 폴더로 가는 메서드가 없다 ③ 조각 읽기 규칙: 라인 없는 포인터는 파일 앞 `snippetLines`(200)줄, 끝이 파일을 넘으면 clamp, **시작이 파일을 넘으면 거부**(`SNIPPET_RANGE_OUT_OF_FILE` → unreadable — 커밋은 불변이라 "낡은 포인터"가 아니라 잘못된 포인터이고, 파일 머리를 대신 보여주면 note와 다른 조각이 근거가 된다. ⚠️ 처음엔 "1부터"였으나 리뷰 4로 변경, 사용자 승인), 0줄 파일은 `SNIPPET_EMPTY`. 파일은 `git cat-file blob`으로 읽는다(`show`는 디렉터리를 트리 목록으로 성공시킨다 — 리뷰 3). **redact를 절단 앞에** 걸고(절단 경계에서 잘린 회사명 접두가 새지 않게) 절단 뒤 `lineRange.end`는 실제 담긴 줄로 다시 센다. 줄·바이트(16KB, 문자 경계) 상한을 넘기면 `truncated` ④ 항목 `date` = 그 커밋의 작성일(`git log -1 --format=%aI`, 읽기만) — 발행정보 `## 근거` 목록에 쓴다 ⑤ 단계는 모델을 쓰지 않으므로 `tokens`·`model`을 비운다(워커가 추정하지 않는다) ⑥ 상한은 `src/evidence/limits.ts` 한 곳 — `maxLinked`는 **담긴 항목** 기준(중복·못 읽는 포인터가 예산을 먹지 않는다) ⑦ **리포 수준 실패는 단계 실패**(`EVIDENCE_REPO_UNAVAILABLE`, 재시도 불가 — 리포 경로가 옮겨졌거나 git이 아니면 근거 0건으로 "성공"하지 않는다, 리뷰 5). 포인터 수준 실패(커밋·경로 없음·바이너리·빈 파일·범위 초과)만 `unreadable`. 저장 실패는 `EVIDENCE_STORE_WRITE_FAILED`(문구에 절대경로 없음, 원인은 `cause`) ⑧ 커밋 메타는 리포·커밋별로 한 번만 읽는다(`CommitMetaResolver` 주입) ⑨ ⚠️ **BE11 착수 전 결정(리뷰 6)**: 본문 입력 "요약"이 번들에 없다(항목별 note뿐) → 번들에 `analyses: { id, kind, title, summary }[]`(인덱싱 때 이미 redact된 값)를 **옵션 필드로 추가**한다(version 1 유지, `parseEvidenceBundle`이 없으면 빈 배열). 구현은 BE11 ⑩ BS5 배선 때 `DATA_DIR`이 비었거나 `BLOG_DIR` 안이면 **기동 거부**(빈 문자열이면 cwd 상대경로가 된다 — 리뷰 20). 기록만: CRLF 파일의 마지막 줄 `\r` 비대칭(원본 일치 원칙상 raw 유지) · 중복 키가 짧은/전체 sha·범위 동치를 구분 못 함 · `safeSegment` 치환 충돌(실데이터는 슬러그·cuid라 안전) · 임시 파일 잔존 시 `remove`가 안 지움 · 삭제 파일 포인터의 `date`는 부모 커밋 날짜(발행정보 표기 규칙 때). discovered(추가 탐색)는 BE9, 뒤 단계가 번들을 읽는 자리(`StepContext.evidence`)는 BE11.
- 2026-09-22 BS2 벨로그 본문 단계 구현에서 정한 것: ① **StepContext에 `modelId`·`sources`**(carried 단계의 출처 runId — 워커가 RunStep.sourceRunId를 옮기고 해석하지 않는다). 본문 단계는 `sources.evidence ?? runId`로 EvidenceStore(DATA_DIR)에서 번들을 직접 읽는다 — 워커가 번들을 로드해 넘기지 않는다(`StepContext.evidence` 자리 제거) ② 번들 `analyses`(옵션, version 1 유지) — 근거 수집이 **항목이 실제로 담긴 글**의 제목·요약만 담는다(BE8 ⑨ 결정 이행) ③ **근거 0개 = 단계 실패**(위 "벨로그 본문" 줄) ④ 모델 호출 실패의 재시도 분류는 `classifyModelError` 한 곳: SDK 오류 `status` 408·409·429·5xx와 네트워크/타임아웃 이름·문구는 재시도, 인증·거부(refusal)·그 밖은 영구. **행에 남는 문구는 고정 문장**(응답 원문·키·절대경로가 새지 않게), 원본은 `cause`로. 빈 출력은 재시도 ⑤ 어투 로더 실패 → `StepFailure` 변환은 `tonePromptFailure` 한 곳(BS3·BS4 재사용, 문구에 파일명만) ⑥ 상한은 `src/steps/limits.ts` `WRITING_LIMITS`(근거 조각 총 120k자 — 넘는 조각은 경로·라인만, 요약 12k자 — 넘으면 제목만, 출력 8192토큰) ⑦ 산출물은 `artifacts['velog.md']`(앞뒤 공백 정리 + 끝 개행), 파일 쓰기는 B3a. 기록만: 프롬프트에 조각 순서는 번들 순(manual → auto)이라 상한에 걸리면 auto 조각부터 빠진다 · 프롬프트 캐싱·시스템 프롬프트 분리 최적화는 실모델 비용을 본 뒤.
- 2026-09-23 BS3 링크드인 단계 구현에서 정한 것: ① **입력은 본문뿐** — 근거 번들·리포 경로·분석 글 자리가 없는 타입(`LinkedinInput`). 근거 대조는 BE10이 본문에 대해 이미 했다는 전제 ② 본문 읽기는 `ArtifactStore.read(slug, sources.velog ?? runId, 'velog.md')`(BS2 ⑧ 규칙의 첫 소비자) — 결과는 이 Run에 쓴다(carried 출처 Run의 폴더는 건드리지 않음) ③ BS2 헬퍼(`tonePromptFailure`·`classifyModelError`·`abortable`·`fenceFor`·`unwrapFence`)는 `velogStep.ts`에서 그대로 import — BS4 Zenn까지 같은 자리, 셋째 소비자가 생기면 공용 파일로 옮길지 결정 ④ 벨로그 링크 자리는 코드가 검사하지 않는다(어투 규칙) ⑤ 실패 코드는 `LINKEDIN_*` 접두로 단계별 분리(타임라인 문구 매핑이 단계를 안다).
- 2026-09-23 BS2 리뷰 반영(reviewer high 3 · med 7 · low 13, 사용자 "추천대로"): ⑧ **ArtifactStore(DATA_DIR)** — 워커는 `StepResult.artifacts`를 저장하지 않으므로(오케스트레이션만) 단계가 EvidenceStore와 같은 패턴으로 `<DATA_DIR>/artifacts/<슬러그>/<runId>/<파일>`에 직접 쓴다. 뒤 단계(링크드인·Zenn·검증)는 `sources.<단계> ?? runId`로 읽고, `posts/<슬러그>/`에는 B3a가 승인·완료 시 복사한다(Run별 이력은 DATA_DIR, posts는 최신본 — run-execution-model). `discard`가 지운다 ⑨ **프롬프트에는 괄호 힌트를 뗀 제목만**(`stripTopicHints`) — 힌트의 리포 별칭이 모델에 가지 않게 ⑩ 조각은 조각 안 최장 백틱보다 긴 펜스로 감싼다(프롬프트 구조 주입 방지). 모델이 전체를 펜스로 감싼 출력은 벗긴다 ⑪ 모델 호출은 signal과 경주(`abortable`) — 어댑터는 signal을 받지 않아 호출은 못 끊지만 단계는 워커 타임아웃·종료 신호에 즉시 반환, 늦은 결과는 버린다(`GenerateInput.signal`은 🔒 인터페이스라 다음 결정) ⑫ **`GenerateResult.truncated`**(🔒 인터페이스 변경, 사용자 승인) — Anthropic `stop_reason max_tokens`·OpenAI `status incomplete`. 본문 단계는 잘린 출력을 `VELOG_OUTPUT_TRUNCATED`(재시도 불가)로 ⑬ SYSTEM 머리는 "사실(수치·시점·동작·결정)은 근거만, 코드·식별자·경로는 어투대로 일반 이름으로 — 바꾸는 것은 이름뿐" — 어투 공통 규칙("회사 고유 경로·필드명은 일반 이름으로")과 충돌하지 않게. **BE10에 기록**: 일반화된 식별자·경로는 unsupported가 아니라 uncertain · 본문이 근거 조각을 N줄 이상 그대로 담으면 flag(클린룸 기계 검사 — 본문 코드의 클린룸은 어투 규칙 + 검수가 보장하고, posts로 가는 velog.md에 조각이 실릴 수 있다는 경계는 여기서 명시) ⑭ 재시도 분류: 연결 오류는 클래스 이름·`error.name`·문구(`ECONNREFUSED` 포함)로, 거부는 어댑터 고정 접두("모델이 응답을 거부했습니다")로만. 기록만(BS5): SDK 자체 재시도(`maxRetries` 기본 2) × 워커 재시도 3(대기 없음) → 429가 최대 9회 — 어댑터 `maxRetries` 명시와 워커 재시도 간 `timers.after` 대기를 BS5 배선 때 결정 · 실모델 스모크는 환경 변수로 켜는 수동 vitest 하나(실제 레지스트리 + tmp 번들 + 실제 어투, 결과·토큰·비용 출력) — BS5 뒤 · 요약 상한은 항목별 판정(뒤의 짧은 요약은 들어간다) · `item.path`는 redact를 안 거친다(디렉터리명은 어투 일반화 규칙에) · 호출당 입력 비용 추정(근거 120k자 ≈ 30~40k 토큰).
