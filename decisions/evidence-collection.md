...
- 근거 수집은 **기본 건너뜀**. 수정 지시에 "근거"·"커밋"·"코드"가 있거나 사용자가 단계 Select로 지정했을 때만 다시 돈다.
- **본문을 재실행하면 근거 검증도 자동으로 다시 돈다.**

## 상태 머신(🔒 B1a, 사용자 작성) 인터페이스 요구사항

이 문서가 상태 머신에 요구하는 것. 구현·타입 이름은 사용자 몫이며, AI는 이 요구사항으로 테스트만 쓴다.
...
3. **`skipped` 상태**: 재실행 시 `evidence`가 건너뛰어지면 이전 산출물을 유지한 채 `skipped`로 표시.
4. **재실행 입력 = (대상 단계, 수정 지시 문자열)**. 규칙: 대상이 `velog`면 `verify`를 자동 포함. `evidence`는 대상으로 명시했거나 지시에 "근거"·"커밋"·"코드"가 있을 때만 포함. 나머지 단계는 기존 원칙(해당 단계만).
