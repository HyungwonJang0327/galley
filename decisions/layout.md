...
- 우: 헤더(주제명 + 상태 배지 + ⋮: 중단 / 처음부터 재실행 / 산출물 폴더 경로 복사) → 타임라인 6줄 → 하단 고정 ActionBar.
  ...
  - **근거 수집 줄**: 보조 텍스트 "연결 n · 탐색 n"(읽지 못한 포인터가 있으면 "· 읽지 못함 n" — 2026-09-27 표기 한국어로). 펼치면 EvidenceBundle 항목 목록(경로:줄 · 커밋 7자 · 날짜 · 출처 · 치환됨/잘림 · 조각 미리보기 3줄). 식별 정보 필터를 안 거친 번들(`filtered=false`)은 조각을 숨기고 안내만.
  - **근거 검증 줄**: 배지 "근거 없음 n · 불확실 n"(unsupported ≥ 1이면 주황). 펼치면 주장 목록 + 상태 + 종류·본문 줄·판정 사유 + 근거 위치(경로:줄 · 커밋, 텍스트 — 링크할 라우트가 없다). 본문 미리보기의 unsupported 밑줄은 Phase 2(evidence-collection).
...
- 2026-09-27 BE13 구현 반영(결정 변경 아님, 표기 조정): 근거 수집 보조 텍스트를 한국어 "연결 n · 탐색 n"으로(표시는 한국어 — db-value-language, 사용자 결정) · "근거 링크" → 근거 위치 텍스트(링크 대상 라우트 없음) · 판정 사유 표시 · 필터 없는 번들은 조각 숨김(사용자 결정). 펼침 힌트 `보기`는 우측 슬롯이 아니라 meta 끝(B2e, navigation.md 2026-09-26). `diff`(재실행 후) 표기는 미구현(todo B2e 이월).
