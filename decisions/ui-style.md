# ⑤ galley-ui 스타일: CSS Modules + 토큰 CSS 변수

## 결정

`galley-ui`는 CSS Modules로 스타일링하고, 색·간격·타이포·라운드·그림자는 `tokens/`의 CSS 변수(`--ui-*`)만 참조한다. 다크는 `[data-theme='dark']` 블록에서 색 토큰만 재정의한다(값은 2026-09-17 확정, 갱신 이력).

## 이유

- 독립 배포 시 소비자 부담 최소: 컴파일된 CSS import만, 별도 config 불필요.
- `sideEffects: ["*.css"]` 규칙과 맞음.
- 하드코딩 색·px 금지 → 토큰 변수로 테마 교체 가능.

## 기각된 대안

- **Tailwind**: 소비자에게 tailwind config·content 스캔 강제 → 디자인 시스템 독립 배포에 부적합.
- **vanilla-extract**: 타입 안전 테마 장점이나 빌드 플러그인·학습 추가. 필요 시 재검토.

## 배포 시 변화

없음(컴파일된 CSS 그대로 소비).

## 결정일

2026-09-08

## 갱신 이력

- 2026-09-08 최초 결정.
- 2026-09-08 (A1) 토큰은 `src/tokens/tokens.css`의 `--ui-*` 변수로. `index.ts`가 로드 → 빌드 산출물 `dist/index.css`, 소비자는 `galley-ui/styles.css`로 import. 정적 토큰이라 단위 테스트 없이 빌드로 검증.
- 2026-09-16 패키지명 치환: 구 스코프 이름 → `galley-ui`(P1b, decisions/package-name.md). 결정 변경 없음.
- 2026-09-17 **다크 값 확정**(P3a, `084dcc1`): `[data-theme='dark']`에 색 토큰 24개 재정의. 본문 대비 4.5:1 이상, accent 같은 계열 밝기 조정, TopBar 라이트 값 유지. 대비표는 worklog 2026-09-17. "자리만"에서 값 있음으로 — 테마 저장·전환 UI는 여전히 앱 범위 밖.
- 2026-09-17 다크 accent-fg를 어두운 글자(#16181c)로(사용자 결정, `e0ff8fb`) — 흰 글자로는 4.5:1이 안 나와서. accent #7b8aff로 상향, 다크는 전 쌍 4.5 이상.
- 2026-09-21 P4-7 Skeleton: 자리표시자 색은 전용 토큰 없이 `--ui-color-border` 재사용(다크도 같은 토큰). 펄스 최저 opacity **0.6** — 0.45는 최저점에서 표면과 거의 구분되지 않아(대비 1.10) 갤러리에서 두 값을 나란히 비교한 뒤 사용자 결정. 결정 변경 없음.
