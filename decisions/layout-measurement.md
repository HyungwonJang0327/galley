...
- 2026-09-12 검사 모듈 `split-pane.mjs` 추가(B2a — 갤러리 2분할 상세형 섹션과 함께, 결정 변경 없음). 105→117 검사. 영역은 `[aria-label="목록 영역"]`·`[aria-label="상세 영역"]`·`[aria-label="하단 액션 바"]`, 우 본문은 `[data-demo="상세 본문"]`의 부모로 찾는다. 잰 것: 좌 폭=토큰 · 좌우 독립 스크롤 · 헤더·하단 바 고정 · 펼쳐도 하단 바가 밀리지 않음. **상호작용 뒤 측정은 프레임을 기다린다** — React 리렌더는 `click()`과 같은 동기 블록이 아니라 그 뒤에 일어나서 바로 읽으면 펼치기 전 값이 잡힌다(`page.evaluate`는 `awaitPromise: true`라 async IIFE가 그대로 된다). 이 스크립트가 "상태 있는 데모를 서버 컴포넌트에 둬서 펼침이 안 됨"을 잡았다(worklog 2026-09-12).
...
- 2026-09-17 검사 모듈 `switch.mjs` 추가(P4-2 — 갤러리 Switch 섹션과 함께, 결정 변경 없음). 125→132 검사. 데모 스위치는 `[role="switch"]` 중 감싼 label 문구("스위치 항목"·"비활성(꺼짐)")로, 토글 횟수는 `[data-demo="switch-count"]`로 찾는다. 잰 것: 트랙 클릭·라벨 클릭 각각 토글 1회 · 트랙 크기=토큰 · 손잡이 여백 좌우·상하 대칭 · disabled 무변화. 이유: happy-dom은 label 활성화를 버블 단계에서 처리해 React 루트의 `preventDefault`보다 먼저 돌아, label로 감싼 Base UI 컨트롤의 **이중 토글을 단위 테스트로 못 잡는다**(happy-dom에서는 항상 2번, 실제 브라우저는 1번). 주의: 대시보드는 `galley-ui` dist를 소비하므로 ui를 고친 뒤에는 `pnpm --filter galley-ui build`를 먼저 돌려야 새 CSS로 실측된다.
- 2026-09-17 검사 모듈 `radio.mjs` 추가(P4-3 — 갤러리 RadioGroup 섹션과 함께, 결정 변경 없음). 132→142 검사. 그룹은 `[role="radiogroup"][aria-label="세로 라디오"|"가로 라디오"]`로, 라벨·보조는 라디오의 `aria-labelledby`·`aria-describedby` id로 찾는다. 갤러리 전제: 세로 데모는 처음에 선택된 항목·긴 보조·disabled 항목이 있고 320px flex 행 상자(`.radioBox`) 안, 가로 데모는 미선택에서 시작. 잰 것: 원=토큰 · 점 여백 대칭 · 원과 라벨 첫 줄 정렬 · 긴 보조 말줄임 · 그룹이 flex 행 부모를 밀지 않음 · 세로 항목 내용 폭 · 라벨 클릭 선택 · disabled 무변화. 이유: 항목 레이아웃(ItemContent)을 Select·Menu와 공유해 그쪽 수정으로 정렬이 조용히 깨질 수 있다.
- 2026-09-17 검사 모듈 `form-field.mjs` 추가(P4-4 — 갤러리 FormField 섹션과 함께, 결정 변경 없음). 142→153 검사. 결합점: `form[aria-label="폼 필드 데모"]` · 컨트롤 `name`(name·note·choice·enabled) · `[data-demo="form-submit-count"]`. 갤러리 전제: 이름 칸은 필수·빈 값으로 시작, 메모(Textarea)는 앱 오류가 있는 필드. 잰 것: Input·Textarea·Select 폭=필드 폭 · Switch 내용 폭 · 오류 테두리 색 · 빈 live 영역 높이 0 · 라벨 클릭 포커스 · required 상속 · 빈 채 제출 차단/채우면 통과. **이 PR에서 셸 스크롤 검사가 실제 버그를 잡았다** — `name` 있는 Base UI 컨트롤의 hidden input(`position:absolute`)이 기준점 없이 문서에 붙어 body를 늘림(→ 컨트롤 래퍼에 `position: relative`). P4-4b Form이 들어오면 제출 검사를 인라인 문구 기준으로 바꾼다.
- 2026-09-17 `form-field.mjs` 제출 검사를 Form 인라인 오류 기준으로 갱신(P4-4b, 결정 변경 없음). 153→159 검사. 결합점에 `input[name="email"]`(Form `errors` 시연) 추가. 갤러리 전제: 이름은 필수·빈 값, 이메일은 `errors`가 걸린 채 시작, 메모·선택의 앱 `error`는 비어 있는/고르지 않은 동안만(고정 error면 제출이 영영 막혀 "통과" 검사가 실패한다 — 이 검사가 그 Base UI 동작을 드러냈다). 스크립트는 메모를 채우고 · 이메일을 바꾸고 · Select 옵션을 하나 골라(합성 pointer 시퀀스, `select.mjs`와 같은 방식) 오류를 모두 푼 뒤 제출한다. 옵션을 못 열면 즉시 error를 반환한다.
- 2026-09-17 검사 모듈 `popover.mjs` 추가(P4-5 — 갤러리 Popover 섹션과 함께, 결정 변경 없음). 159→174 검사. 결합점: 트리거 글자 "팝오버 기본"·"팝오버 오른쪽"·"팝오버 화면 하단"(마지막은 페이지 맨 아래 카드 — Select·Menu와 공유), 팝업은 트리거의 `aria-controls`로 찾는다. 갤러리 전제: "팝오버 기본" 본문에는 포커스 가능 요소가 없다(팝업 자체의 포커스 링을 잰다). **`cdp.mjs`의 page에 실제 입력 헬퍼 추가**: `mouseClick(x, y)`(CDP `Input.dispatchMouseEvent`) · `pressKey(key)`(`Input.dispatchKeyEvent`). 이유: `:focus-visible`은 브라우저가 기억하는 직전 입력 종류(포인터/키보드)에 달려 있어 `el.click()`·합성 이벤트로는 "마우스로 열면 링 없음 / 키보드로 열면 링 있음"을 구분할 수 없다. 입력 종류에 달린 동작을 잴 때는 이 헬퍼를 쓴다. 참고: `cdp.test.mjs`는 vitest로 돈다(`node --test` 아님).
- 2026-09-17 검사 모듈 `inline-alert.mjs` 추가(P4-6 — 갤러리 InlineAlert 섹션과 함께, 결정 변경 없음). 174→186 검사. 결합점: `[data-demo="alert-tones"]`(info·success·warning·danger 순서) · `[data-demo="alert-multiline"]`(제목+여러 줄 본문+action, 420px 상자) · `[data-demo="alert-plain"]`(마지막은 ListRow 안의 plain 알림). 잰 것: tone별 live 영역 role · 루트에 role 없음 · tone별 배경 · 아이콘=첫 줄 중심 · action이 live 밖·오른쪽 끝·눌리지 않음 · 상자가 부모 폭 안 · plain의 투명 배경·tone 글자색 · **목록 행 높이를 늘리지 않음**(알림을 잠깐 숨겨 기준 높이를 잰다). 마지막 검사가 추가 당일 회귀를 잡았다 — `flex-wrap` + 본문 기준 폭 때문에 좁은 자리에서 본문이 아이콘 아랫줄로 내려감.
