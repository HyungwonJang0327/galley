...
- [x] **레이아웃 실측 CI 통합**(planning 미결 /decide 2-B → A, ci/layout-job): decisions/layout-measurement.md 결정 변경(수동 → CI `layout` 잡, 2026-09-10 `c209053`) · `ci.yml` `layout` 잡(`verify`와 병렬 + 실패 시 `layout-screenshots` artifact, 2026-09-10 `6c262df`) · CI 1차 실패(Linux 프로필 정리 ENOTEMPTY) fix `removeDir` + 테스트(2026-09-10 `164467e`) · 2차 실패(러너 한글 폰트 없음) `fonts-nanum` 설치(2026-09-10 `31b01b4`). PR #45. required check 등록은 초록 확인 뒤 사용자 몫.
