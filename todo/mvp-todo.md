...
- [x] **셸·홈 정적 프리렌더 fix**(AH4 중 발견, feat/dashboard-next-run): Next가 `(dashboard)` 레이아웃 사용 라우트(홈·/runs·/publish·/settings/\*)를 빌드 시점 큐 값으로 프리렌더해 주제_큐.md를 고쳐도 화면이 그대로였다. `export const dynamic = 'force-dynamic'` (2026-09-12 `0e42f52`). 단위 테스트로는 안 잡히고 픽스처 실측이 잡음 — decisions/navigation.md 갱신 이력.
...
- [x] **AH4** (2026-09-12 `3cf57b8`·`9728c15`·`0e42f52`·`bc666ce`, feat/dashboard-next-run) fe — "다음 실행" 카드(대기 큐 A4d 데이터 연결: 맨 위 1개 크게 + 2~4위 작은 행, `지금 실행`은 비활성+사유(BM6 전), 빈 상태 EmptyState + `후보 보기`). 작은 행은 A5a `ListRow` 재사용. 사용자 결정: 전체 폭(좌우 분할은 AH5) · 보조 정보는 카테고리만. 곁들여 ui `Button`에 `render`(버튼 모양 링크) 추가 + 갤러리 예시, 셸 정적 프리렌더 `fix`. 커밋: `feat(dashboard): 홈에 다음 실행 카드 추가`
