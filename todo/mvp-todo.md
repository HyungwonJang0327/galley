...
- [ ] **BX1** pl — 큐 파서: `QueueTopic.series?: { key, episode }`(태그 뗀 `title`), 라이터가 태그를 다시 붙임(완료 줄은 `날짜 [A-1] 제목`), `ParsedQueue.seriesDefs`(후보 `### 시리즈` 아래 정의 줄, **라이터가 같은 자리에 되돌림** — 지금은 되쓰기 한 번에 사라진다), `moveTopic`이 `series`를 유지, `stripTopicHints`·`normalizeTopicTitle`이 태그를 먼저 뗌. 픽스처 `__fixtures__/series.md`(실제 파일 복사, 정의 7·태그 34). 브랜치 `feat/queue-series-parse`. 커밋: `feat(queue): 시리즈 태그 파싱과 정의 줄 보존` / `test(queue): 시리즈 fixture 라운드트립` / `feat(queue): 매칭 키에서 시리즈 태그 제외`
