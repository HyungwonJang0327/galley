...
- [x] **RF3** (2026-09-21 `036c725`–`3271ade` 25커밋, refactor/ui-pre-release) ui(+fe: dashboard 따라가기) — 승인 항목당 커밋 하나(`refactor(ui): …`, dashboard 수정은 `refactor(dashboard): …`). 완료: `pnpm lint && pnpm typecheck && pnpm test` · `pnpm --filter galley-ui build` · `verify:layout` · dashboard `next build` · 갤러리 스크린샷 전후 동일. worklog에 승인/기각 판단 기록(/review 규칙).
