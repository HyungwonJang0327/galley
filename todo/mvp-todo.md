...
- [ ] **P4-11** fe — RunActionBar의 확인 Dialog 2개(재실행·승인)를 `useConfirm`으로 치환(P7a에서 앞당김, 2026-09-15 사용자 결정 — 훅 API를 실제 소비자에서 검증한 뒤 배포). `useState` 두 벌·Dialog JSX 제거, 테스트 9건은 동작 유지 확인. 브랜치 `refactor/dashboard-use-confirm`. 커밋: `refactor(dashboard): RunActionBar 확인 Dialog를 useConfirm으로` **P4-9 리뷰 L7 입력(2026-09-21)**: 현재 RunActionBar Dialog와 안 맞는 것 셋 — ① footer 버튼 `size="sm"`(useConfirm은 md) ② `disabled={pending}`으로 액션이 도는 동안 Dialog를 열어 둠(useConfirm은 확인 즉시 닫힘 → 동작 변화, 바 쪽 pending 표시로 대체) ③ 재실행 Dialog의 `children`(계획 `<dl>`) — `description`은 `<p>`라 `<dl>`을 넣으면 잘못된 중첩. 방향: 승인만 useConfirm, 재실행은 `useAwaitDialog<boolean>` 직접(children 유지) 또는 ConfirmOptions에 `children?` 추가 — 착수 때 결정.
