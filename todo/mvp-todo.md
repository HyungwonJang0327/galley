...
- [x] **UM3** (2026-09-14 `206a298`, feat/dashboard-run-detail) ui — `Input` 프리미티브(Base UI Input 래퍼, `invalid` prop, 폭은 부모, Select 트리거와 같은 높이·포커스 링). 갤러리 섹션 + SplitPane 데모 input 교체. B2c 리뷰 9번에서 파생. 테스트 5. `Textarea`는 B2d에서 필요하면. 커밋: `feat(ui): Input 프리미티브 추가`
...
- [ ] 🔒 **B2c** fe — **직접 작성 핵심 모듈 (a')**(2026-09-12 교체 — decisions/core-modules.md). AI는 테스트·리뷰만. 2분할 화면: 좌 목록(검색·탭 실행중/완료·"승인 대기만" 체크) / 우 타임라인(**6단계** 순서 고정). 커밋: `feat(dashboard): 실행 상세 2분할 화면 추가` — 2026-09-14 `/runs` 500 수정(`e513d09` RunPoller가 `active`만 받음) + `server-only` 경계(`2b207d6`, decisions/server-only-boundary.md). 2026-09-14 리뷰 14건 반영 — 항목당 커밋(`4024277`…`d769ecc`, worklog/2026-09-14.md 표). 11은 B2e, 5의 "근거 없음 n"은 BE13으로. PR 대기.
