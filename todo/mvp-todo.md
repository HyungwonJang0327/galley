...
- [x] **P4-8** (2026-09-21 `a2b7e8f`·갤러리 `135f4a3`·실측 `f60d817`·리뷰 커밋 `014bf0c`–`05cc50e`, feat/ui-toast) ui — **Toast**(primitives, Base UI Toast) — `ToastProvider`(뷰포트 위치 top-right|bottom-right, 최대 개수) + `useToast()` 훅(`toast({ title, description?, tone?, duration? })`). 훅은 `hooks/`에 두고 배럴 export. 키보드 접근(F6/포커스 이동)·`role=status`·수동 닫기 버튼은 Base UI 기본. Base UI에 Toast가 없으면 멈추고 직접 작성 범위 보고(2026-09-15 확인: 있음). 브랜치 `feat/ui-toast`. 커밋: `feat(ui): Toast 프리미티브와 useToast 훅 추가`
