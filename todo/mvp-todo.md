...
- [x] **셸·홈 정적 프리렌더 fix**(AH4 중 발견, feat/dashboard-next-run): Next가 `(dashboard)` 레이아웃 사용 라우트(홈·/runs·/publish·/settings/\*)를 빌드 시점 큐 값으로 프리렌더해 주제_큐.md를 고쳐도 화면이 그대로였다. `export const dynamic = 'force-dynamic'` (2026-09-12 `eb1e36b`, PR #68). 단위 테스트로는 안 잡히고 픽스처 실측이 잡음 — decisions/navigation.md 갱신 이력.
- [x] **실측 Select 팝업 스크롤 대기**(AH2 중 발견, feat/dashboard-home-nav): Base UI가 팝업을 연 뒤 비동기로 선택 항목까지 스크롤하는데 `select.mjs`가 그 전에 측정 → 페이지가 길어지자 "[항목 40개] 열릴 때 선택 항목이 보임" 2회 실패(제품 회귀 아님). `popup.scrollTop` 안정 대기 추가 (2026-09-12 `0b171e7`, PR #64). decisions/layout-measurement.md 갱신 이력.
...
- [x] **AN7** (2026-09-12 `efc52a1`·`b947237`, feat/queue-reload-button, PR #56) fe — 큐 화면 헤더 `파일에서 다시 불러오기` 버튼(2026-09-12 사용자 요청·결정, AN5 앞). Server Action이 주제_큐.md→DB 재적재(자동 적재와 같은 경로) 후 `revalidatePath('/', 'layout')`, 섹션별 개수·실패 사유를 한 줄 표시. AI 호출 없음. 커밋: `feat(queue): 큐 수동 갱신 액션 추가` · `feat(queue): 큐 화면에 파일에서 다시 불러오기 버튼 추가`
  - 완료조건: 파일을 밖에서 고친 뒤 버튼을 누르면 화면 이동 없이 목록·탭 개수가 파일과 같아지고 결과 문구가 뜬다. 자동 적재는 유지.
...
- [x] **AH3** (2026-09-12 `cf2dc5a`·`2f6fb92`·`6a2faa1`·`65787ed`, feat/dashboard-home, PR #66) fe — 홈 페이지 골격: `app/(dashboard)/page.tsx`(기존 `app/page.tsx` redirect 삭제) · h1 "홈" · 다음 스케줄 시각(수·토 18:00 중 가까운 쪽, TZ .env, **계산 apps/dashboard 유틸**) · StatTile 4개. 카운트는 배지와 같은 소스(`lib/nav-counts`, 대기 n은 A4d `QueueItem`, 승인 대기·발행·비용은 더미/"—"). 갱신=서버 렌더+네비게이션. 커밋: `feat(dashboard): 홈 페이지 골격과 요약 타일 추가`
...
- [x] **AH4** (2026-09-12 `b801ac4`·`7b6b92e`·`eb1e36b`·`5b310a2`, feat/dashboard-next-run, PR #68) fe — "다음 실행" 카드(대기 큐 A4d 데이터 연결: 맨 위 1개 크게 + 2~4위 작은 행, `지금 실행`은 비활성+사유(BM6 전), 빈 상태 EmptyState + `후보 보기`). 작은 행은 A5a `ListRow` 재사용. 사용자 결정: 전체 폭(좌우 분할은 AH5) · 보조 정보는 카테고리만. 곁들여 ui `Button`에 `render`(버튼 모양 링크) 추가 + 갤러리 예시, 셸 정적 프리렌더 `fix`. 커밋: `feat(dashboard): 홈에 다음 실행 카드 추가`
...
- [x] **A5c** (2026-09-11 `ba55e00`·`e62b4ac`, AN4와 한 PR #51) fe — 큐 데이터 페칭(pipeline `loadQueueSections`: 요청마다 파일→DB 재적재 후 섹션별 읽기) → AN4 탭 + A5a 행에 공급. 선행으로 env 위치 결정(루트 `.env` — decisions/env-location.md, `60aaabb`·`32bd3b6`). (기존 "3화면" 폐지 — 1화면+탭은 AN4.) 커밋: `feat(dashboard): 큐 데이터 페칭 배선`
...
- [x] **A6e** (2026-09-13 `911069c`·`5739f7e`·`f813d14`, feat/queue-missing-confirm, PR #86) fe+pl — **"파일에서 사라짐" 확인 UI**: `missingSince`가 찍힌 항목(= Run이 붙어 있어 자동으로 내리지 않은 것)을 큐 화면에 띄우고 `보류로 옮기기` / `그대로 두기`를 받는다. 제목 오타 수정이 파서에겐 "삭제 + 추가"로 보여 이력이 갈라지는 것을 사람이 막는 자리다. 보류 탭은 `holdReason`으로 `removed-from-file`과 `manual`을 구분해 보여준다(되살릴지 판단). decisions/queue-sync-direction.md "사라진 줄 처리". 커밋: `feat(queue): 파일에서 사라진 항목 확인 UI 추가`
...
- [x] **B1f** (BW2 워커 루프가 담당 — 2026-09-13 완료, 2026-09-22 정리) pl — 단계 실행 오케스트레이션(별도 프로세스 — decisions/run-location.md).
...
- [x] **UM4** (2026-09-14 feat/ui-form-primitives, PR #93 `ecc82b8`·`35f60ca`·`d39394b`·`5c55ebe`·`b11b498`) ui — 폼 프리미티브 4개: `Textarea`(native, rows 3, invalid) · `Checkbox`(Base UI, 라벨 클릭 토글, indeterminate, 토큰 `--ui-checkbox-size`) · `Tooltip`(trigger 요소, side, 다크 pill, `role="tooltip"` 명시, 토큰 `--ui-tooltip-max-width`) · `Tabs`(페이지 안 상태 탭, count·disabled·content). 갤러리 섹션 4개 + 툴바 맨 input·select 교체. 테스트 22. B2d(Textarea)·좌 목록 "승인 대기만"(Checkbox)·접힌 사이드바(Tooltip)가 소비. 커밋: `feat(ui): Textarea 프리미티브 추가` 외 4.
- [x] **UM3** (2026-09-14 `b9e0d2e`, feat/dashboard-run-detail) ui — `Input` 프리미티브(Base UI Input 래퍼, `invalid` prop, 폭은 부모, Select 트리거와 같은 높이·포커스 링). 갤러리 섹션 + SplitPane 데모 input 교체. B2c 리뷰 9번에서 파생. 테스트 5. `Textarea`는 B2d에서 필요하면. 커밋: `feat(ui): Input 프리미티브 추가`
...
- [x] **BW2** (2026-09-13 `aad012c`·`e32bfae`·`2a48335`·`536a542`·`c9616a7`·`e1026e6`, feat/worker-tick, PR #87) pl — 워커 루프. **오케스트레이션만** 하고 단계 내용은 `StepRunner` 뒤(모든 단계가 모델 호출이 아니다) — Mock으로 토큰 없이 CI에서 돈다. `runOnce(deps)`는 비결정성을 전부 deps로 밀고(**clock·ids**) **한 tick = 한 단계**, 반환 `{ outcome, stepRef? }`. 잡는 틱을 따로 떼 승인·취소가 끼어들 틈을 준다. 클레임은 조건부 `updateMany`+count로 원자적, **검수 상태가 `실행 중`인 것만** 집어간다(승인 대기를 다시 집어가던 것을 통합 테스트가 잡음). `bin/worker.ts`는 타이머·SIGTERM·종료만. **Node 24 타입 스트리핑으로 빌드 없이 실행**(decisions/node-runtime.md). pipeline 168→200. 커밋: `feat(pipeline): 워커 프로세스 실행 루프 추가`
- [x] **BW3** (2026-09-13 `97bb6d1`·`03fc5a4`·`bef7865`·`a4d0027`, feat/worker-recovery, PR #90) pl — 중단 감지·재개. 회수(heartbeat 30s 공백→interrupted→완료 단계 다음부터, 매 틱 검사 = 기동 시+깨어날 때)는 BW2에 이미 있었고, 이번에 채운 것: ⑴ **단계 도중 heartbeat 5s**(타이머를 `WorkerDeps.timers`로 주입 — runOnce는 setInterval 직접 호출 금지; StepRunner 협조에 기대지 않는다) ⑵ **단계 타임아웃 10분**(`STEP_TIMEOUT_MS`, heartbeat와 별개 — 사용자 결정 — `STEP_TIMEOUT` 재시도 가능) ⑶ **종료 신호에 클레임 반환**(`released`): 전에는 취소가 실패로 흘러 3번 재시도 뒤 `failed`가 됐다. 이제 `StepRunner.discard`로 쓰다 만 산출물을 버리고 단계 pending(시도 횟수 미집계)·실행 interrupted로 되돌려 다음 기동이 바로 잡는다 ⑷ **프로세스 스모크**(`bin/worker.smoke.test.ts`, `test:smoke`, 기본 스위트 제외) + CI `smoke` 잡(required, 흔들리면 informational). pipeline 235→245 + 스모크 1. 커밋: `feat(pipeline): 워커 중단 감지와 단계 재개 추가`
...
- [x] **BE4** (2026-09-22 `698b3d6`·`94b681e`·`28eb236`·`ee83a7e`·`179b6ab`·`97cef4e`·리뷰 수정 `f6dba72`·`0afb0f4`·`170dace`·`379a650`·decisions `54a8054`, feat/repo-indexer-changes — ~~IndexJob에 기록~~ → 토큰·비용은 report로 돌려주고 **IndexJob 기록은 BE5 실행자**(결정 ⑧과 일관)) pl — 리포 인덱서 2: `git log`를 기간(월)·경로로 묶어 `change` 분석 글 + `overview`. 커밋: `feat(pipeline): 리포 인덱서에 변경 이력 분석 추가`
  - 완료조건: BE3와 동일 포인터 검증 + change 분석 글에 period가 있다.
- [x] **BE5** (2026-09-22 `452cb4f`·`2255124`·`3cde2ca`·`0c82b66`·`a1a5953`·`b214855`·리뷰 수정 `0ae7162`·`81b4b76`·`5587762`·`cbef7d9`·`774b54c`·decisions `76d3f90`·`988203c`, feat/index-job-runner — 한 틱 = 배치 하나(Run 우선), 작업은 클레임 시점 `toSha` 고정, IndexJob 쓰기는 소유 조건, CLI는 등록+큐잉만) pl — 증분 재인덱싱(HEAD 비교 → `stale`, 바뀐 경로·새 커밋 범위만) + `--full` + CLI `bin/index.ts`(`pnpm --filter @galley/pipeline index <path> [--name --alias --read-only --model --full]`, 모델 기본 = 레지스트리 `indexingDefault` Haiku 4.5, 사용 모델을 `Repo.lastIndexModelId`에 기록). 커밋: `feat(pipeline): 증분 재인덱싱과 index CLI 추가`
...
- [x] **BE13** (2026-09-27 `7167c7e`·`aadd726`·`3df3426`·`06869a3` + 리뷰 `918d333`·`12d0f44`·`564fa05`, feat/timeline-evidence-verify) fe — 실행 상세 타임라인에 근거 수집("연결 n · 탐색 n", 펼침 항목 목록·조각 3줄)·근거 검증("근거 없음 n · 불확실 n" 주황 배지, 펼침 주장 목록·판정 사유·근거 위치) 표시 + 좌 목록 "근거 없음 n". 홈 "지금 할 일"은 **AH5에서**(카드 미구현). 커밋: `feat(dashboard): 타임라인에 근거·검증 결과 표시`
  - 이월(리뷰, 기록만): M1 목록 행마다 verification.json 읽기 상한 없음(현행 유지 — AH5 때 RunStep에 수 기록 결정) · L4 검증 배지가 토글 버튼 밖 · L5 Badge tone을 클래스명으로 판정하는 테스트 · L7 슬러그 변경 시 출처 Run 파일이 옛 폴더(B3a M3와 같은 뿌리).
...
- [x] **BE14b** (2026-09-12 `e10a27a`·`7800db6`·`9b86048`, feat/rerun-cascade, PR #84) pl — `planRerun(input)` → `{ startStep, fresh, carried }`(순수 함수, DB 미접근). 시작 단계는 Select 지정 우선 → 없으면 지시에 "근거·커밋·코드"면 `evidence`·아니면 `velog`. `RerunPlan.skipped`·단계별 특례 제거. **출처는 분리**: `resolveCarriedSources(prisma, prevRunId, steps)` → `Map<Step, runId>`(리포지토리) — 직전 Run 행이 `fresh`면 그 Run, `carried`면 그 행의 `sourceRunId`를 **승계**(반복 재실행에서 출처가 한 칸씩 밀리는 것 방지). pipeline 124→136.
...
- [x] **BS2** (2026-09-23 `1a36bf2`·`9468e1a`·`8d833b5`·`96febfe`·리뷰 수정 `c288c95`·`bd7b7dd`·`9f3d35e`·`eab76d8`·decisions `e8c8cd9`+리뷰 반영, feat/velog-step — ArtifactStore(DATA_DIR)에 단계가 직접 저장, 근거 0개는 실패, `truncated` 🔒 변경) pl — 벨로그 본문 단계(`velog`): 입력 = 주제 + EvidenceBundle + 어투 프롬프트 + 수정 지시(재실행 시) → ModelAdapter 호출 → `artifacts.velog`(마크다운) + tokens·costUsd·model·promptHash. 입력 타입은 BE11 그대로(리포 경로·분석 글 원문 자리 없음). 커밋: `feat(run): 벨로그 본문 단계 구현`
...
- [x] **BS3** (2026-09-23 `bc54e48`·decisions `977b684`, feat/linkedin-step — 본문 없음·빈 본문은 `LINKEDIN_BODY_MISSING`·`LINKEDIN_BODY_EMPTY`, 링크 자리는 코드가 검사 안 함) pl — 링크드인 단계(`linkedin`): 입력 = 벨로그 본문(직전 succeeded `velog` 산출물) + 어투 프롬프트 → `artifacts.linkedin`. 본문에서 파생만(EvidenceBundle 재입력 없음). 커밋: `feat(run): 링크드인 요약 단계 구현`
  ...
  - 완료조건: velog 산출물이 없으면 실패(`LINKEDIN_BODY_MISSING`, 재시도 불가). Mock으로 tokens·promptHash 기록.
- [x] **BS4** (2026-09-23 헬퍼 이동 `ceb4782`·상한 `aaac9dd`·단계 `8afa3ed`·배럴 `82597e5`·decisions `dffbe92`, feat/zenn-step — frontmatter는 코드, `published: false` 상수, H1→title, topics 비움) pl — Zenn 일본어판 단계(`zenn`): 입력 = 벨로그 본문 + 어투 프롬프트 → `artifacts.zenn`(frontmatter `published: false` 고정, title·emoji·type·topics). 커밋: `feat(run): Zenn 일본어판 단계 구현`
  - BS3 리뷰에서 넘어온 것: **첫 커밋 = 공용 헬퍼(`tonePromptFailure`·`classifyModelError`·`abortable`·`fenceFor`·`unwrapFence`)를 `steps/writing.ts`로 이동**(동작 변화 없음, 배럴 정리 포함) · 러너 골격 팩토리로 합칠지 그때 결정 · SYSTEM 머리는 링크드인처럼 "남은 회사 고유 이름은 어투대로 일반화" · limits·배럴은 별도 커밋.
  - 완료조건: 산출물 frontmatter에 `published: false`가 항상 있다(테스트). 모델 출력이 frontmatter를 만들어도 덮어쓴다.

### BX. 시리즈 대응 — 파일 표기 `[A-1]` + 후보 `시리즈 A.` 정의 줄 (decisions/series.md · 2026-09-24 추가, BS5 앞)

주제_큐.md에 시리즈 항목이 들어왔다(편 줄 `- [A-1] 제목`, 후보 `### 시리즈` 아래 정의 줄 `시리즈 A. <이름> (<메모>)`). 스케줄 프롬프트(월·수·금)가 같은 규칙으로 파일을 읽으므로 Galley도 같은 결과를 내야 한다. 비목표: 시리즈 편집 UI(이름·순서 변경 — 파일에서 손으로), 벨로그 시리즈 등록 자동화(체크리스트 항목으로만). 순서 BX1 → BX2 → BX3 → BX4 → BX5 → BX6, 항목 = 브랜치 하나.

- [x] **BX1** (2026-09-24 `5ae088a`·`82dfa82`·`8c7889a`·`207da68` + 리뷰 `d335937`·`3f6338b`·`8bd3133`, feat/queue-series-parse, PR #132) pl — 큐 파서: `QueueTopic.series?: { key, episode }`(태그 뗀 `title`), 라이터가 태그를 다시 붙임(완료 줄은 `날짜 [A-1] 제목`), `ParsedQueue.seriesDefs`(후보 `### 시리즈` 아래 정의 줄, **라이터가 같은 자리에 되돌림** — 지금은 되쓰기 한 번에 사라진다), `moveTopic`이 `series`를 유지, `stripTopicHints`·`normalizeTopicTitle`이 태그를 먼저 뗌. 픽스처 `__fixtures__/series.md`(~~실제 파일 복사~~ → **합성**: 실제 파일 복사가 auto mode에서 거부돼 사용자 결정 — 구조만 동일, 정의 7·태그 18, `.prettierignore`). 브랜치 `feat/queue-series-parse`. 커밋: `feat(queue): 시리즈 태그 파싱과 정의 줄 보존` / `test(queue): 시리즈 fixture 라운드트립` / `feat(queue): 매칭 키에서 시리즈 태그 제외`
  - 완료조건: pipeline 테스트 초록 · 픽스처 parse→write가 **바이트 동일**(정의 줄 앞 빈 줄 포함)이고 write→parse 구조 동일 · 태그 없는 제목의 매칭 키 불변 테스트 · 태그 있는 기존 DB 행(제목에 태그 포함)이 새 키로 매칭돼 id 유지 테스트 · reviewer 통과.
- [x] **BX2** (2026-09-24 `2f69150`·`4fc6680`·`7d7bc3f` + 리뷰 `af74942`·`eba78ca`·`454ac33`·`cead767`·`fb0eba9`, feat/queue-series-columns, PR #133) pl — DB 적재: `QueueItem.seriesKey String?`·`episodeNo Int?`(마이그레이션 1개, `@@index([seriesKey, episodeNo])`), `parsedQueueToRows`가 채움(재적재 시 재생성, 정의 줄은 DB에 안 넣음), `isUnchanged` 비교에 포함. `getSeriesContext(seriesKey)` → `{ key, name, nameJa?, episodes: [{ episodeNo, title, status, slug?, velogUrl?, alreadyPublished }] }`(이름은 파일 seriesDefs, 편은 DB 세 상태 합침, 슬러그는 완료 줄 `(posts/<슬러그>)`, velogUrl은 완료 줄의 `https://velog.io/...`, `(기존 글)`은 alreadyPublished). 브랜치 `feat/queue-series-columns`. 커밋: `feat(db): QueueItem에 seriesKey·episodeNo 추가` / `feat(queue): 시리즈 컨텍스트 조회 헬퍼`
  - BX1 이월 처리: L3 URL은 필드 분리 대신 컨텍스트가 제목에서 읽음(`trailingUrl`, 파일 라운드트립 불변) · M3 복원 태그 재부착 `4fc6680` · L5 배럴 export `7d7bc3f` · **L1 정의 줄 원문 보존은 미반영**(괄호 둘인 이름의 `ja:` 분리 오류는 알려진 한계 — 드묾).
  - 리뷰 결정(2026-09-24 사용자): `getSeriesContext`는 **읽기 전용**(적재 안 함, 편은 파일 줄 네 섹션·직접 보류 포함, DB는 id 조회만 — `topicId?`). decisions/series.md 데이터 흐름.
  - 완료조건 확인(2026-09-25): migrate deploy 후 사라진 줄 경고 0건(사용자 화면·DB 조회). 첫 적재에서 태그 행이 한 번 일괄 update돼 자동 연결이 한 번 재계산된다.
  - 완료조건: importQueue.db·loadQueue.db에 시리즈 케이스 · 마이그레이션 적용 후 실제 큐 재적재 시 사라진 줄 경고 0건(사용자 확인) · 머지 보고에 "마이그레이션 있음 → migrate deploy" 명시.
- [x] **BX3** (2026-09-25 `ff54b08`·`4468990`·`566e44b`·`aeb8ff4` + 리뷰 `39270cd`·`6f79ae3`·`741cd49`·`6f02eef`, feat/series-writing, PR #134) pl — 글쓰기 단계: `writing.ts`에 `renderSeriesHeader`·`renderNextEpisode`·`stripSeriesLines`(순수, 1편/중간/마지막/이전 편 `(기존 글)` 4케이스), `StepContext.series?: SeriesContext`(워커가 `WorkerDeps.seriesContext(topicId)`로 채움 — bin/worker.ts에 Storage(BLOG_DIR) 배선), velog: 첫 H1 다음 줄 헤더 + `## 결과` 절 끝 다음 편 줄(코드가 붙임), SYSTEM 머리 한 줄, `(기존 글)` 편 입력이면 `SERIES_EPISODE_ALREADY_PUBLISHED`(재시도 불가). linkedin: 입력 본문에서 시리즈 줄 제거. zenn: 제거 + `## はじめに` 첫 문장 앞 「〜シリーズの第N回です。」(시리즈명 일본어는 정의 줄 메모 `ja:` — 없으면 한글명). 브랜치 `feat/series-writing`. 커밋: `feat(pipeline): 시리즈 안내 렌더 함수와 StepContext.series` / `feat(run): 벨로그 단계 시리즈 헤더·다음 편 삽입` / `feat(run): 링크드인·Zenn 단계 시리즈 줄 처리`
  - 구현 요약: 렌더 함수는 `steps/series.ts`(writing.ts 대신 — 순수 함수 묶음), 워커 배선은 `WorkerDeps.series`(= `createSeriesSource`, 계획의 `seriesContext`). 발행 제목 `| 시리즈명 N편`·`(第N回)` 확정(decisions/series.md). `(기존 글)` 차단은 velog가 아니라 워커 모든 단계(리뷰 M5).
  - 이월: 워커는 아직 Mock 단계 러너 — 실제 산출물 적용은 BS5 라우팅 뒤 · 스케줄 프롬프트 1-1절에 같은 제목 표기 반영(사용자, Galley 밖) · H1 없는 출력의 제목 접미사 누락(L3).
  - 완료조건: velog·linkedin·zenn 테스트에 시리즈 케이스, 시리즈 아닌 기존 테스트 전부 불변, reviewer 통과.
- [x] **BX4** (2026-09-25 `2112cfb`·`2175932`·`7797c37`·`e683354` + 리뷰 `22d3326`·`b817e37`, feat/series-publish-info, PR #135) pl — 발행정보·완료 이동: `renderSeriesPublishSection(ctx)` → `## 벨로그 시리즈`(`시리즈명 · N/M편`, 시리즈 아니면 없음 — B3a가 소비), `completeTopic(queue, …)`(순수 — 완료 줄 `- YYYY-MM-DD [A-1] <글 제목> (posts/<슬러그>)`, 태그 유지, 스케줄 결과와 동일 형식; 지금 완료 이동 코드는 없고 B3a·Phase 2가 씀), 발행 체크리스트 타입 `PublishChecklistItem` + `seriesChecklist(ctx)`("벨로그 시리즈에 추가"·"이전 편 링크 자리표시자 채움", 시리즈 편만 — 화면은 Phase 2). 브랜치 `feat/series-publish-info`. 커밋: `feat(queue): 완료 이동 시 시리즈 태그 유지` / `feat(pipeline): 발행정보 벨로그 시리즈 절`
...
- [x] **BX5** (2026-09-26 `01586a2`·`9905410`·`ba6c77c`·`5035fc8` + 리뷰 `9803f65`·`a939030`·`fc6a526`·`6b83236`, feat/queue-series-ui, PR #136) fe — 큐 화면: 행 제목 앞 Badge(`A-1`, 중립 톤) + Tooltip(시리즈명 · N/M편), 태그는 제목 텍스트에 안 섞음(정렬·검색은 제목). 후보 탭 `### 시리즈` 아래 정의 줄을 그룹 헤더로(편이 대기로 옮겨져 비면 "편 없음 · 대기 N편"). `(기존 글)` 편은 실행 버튼 비활성 + "이미 발행된 글". 실행 상세 상단 "○○ 시리즈 N/M편" + 이전·다음 편 제목. 새 ui 컴포넌트 금지(Badge·Tooltip로, 부족하면 보고). 서버 컴포넌트가 판정, 클라에는 props만(server-only 경계). 브랜치 `feat/queue-series-ui`. 커밋: `feat(dashboard): 큐 목록에 시리즈 배지·그룹 헤더` / `feat(dashboard): 실행 상세에 시리즈 편 정보`
...
- [x] **P1a** (2026-09-16 `2bf988e`, chore/ui-rename) ui — 옛 `@galley` 스코프명 → `galley-ui`. 시작 전 `grep -rn --exclude-dir=node_modules --exclude-dir=dist --exclude-dir=.git --exclude-dir=.next -F 'galley-ui' .` 목록 제시. 코드·설정: `packages/ui/package.json` name · `apps/dashboard/package.json` 의존성 · dashboard import 전부(`galley-ui`, `galley-ui/styles.css`) · `eslint.config.js` no-restricted-imports 패턴 · `.github/workflows/ci.yml` `--filter` · `apps/dashboard/next.config.mjs` · `.claude/settings.local.json` · `.changeset/config.json`(ignore 목록 확인) → `pnpm install`로 lock 재생성. 완료 조건: `pnpm lint && pnpm typecheck && pnpm test && pnpm --filter galley-ui build` 통과 + dashboard `next build` 통과. 커밋: `chore(ui): 패키지명 galley-ui로 변경`
- [x] **P1b** (2026-09-16 `71ef83b`, chore/ui-rename) doc — 문서·에이전트·커맨드 치환: CLAUDE.md · README.md · planning.md · INTENT.md · COMMIT_CONVENTION.md(scope 표) · decisions/_(본문 치환 + 갱신 이력 한 줄) · todo/_ · .claude/agents/* · .claude/commands/* · .changeset/README.md. **worklog/는 날짜별 기록이라 치환하지 않는다.** 완료 조건: `grep -F 'galley-ui'`가 worklog/ 밖에서 0건. 커밋: `docs(docs): 패키지명 변경을 문서·에이전트에 반영`

### P2. 배포 메타데이터 — 브랜치 `chore/ui-publish-meta`

- [x] **P2a** (2026-09-17 `2d0a68e`, chore/ui-publish-meta) ui — `packages/ui/package.json`: `private` 제거(version 0.0.0 유지) · `description`(영어 한 줄) · `license: MIT` · `repository{type,url: git+https://github.com/HyungwonJang0327/galley.git, directory: packages/ui}` · `homepage`(리포 packages/ui 경로) · `bugs` · `keywords`(react, design-system, ui-kit, admin, dashboard, base-ui, css-modules) · `author` · `publishConfig{access: public}` · `engines.node >=20`(소비자 기준. 리포 engines >=24와 다름은 README에만). `sideEffects`·`exports`·`files` 그대로. 루트 `LICENSE`(MIT, Hyungwon Jang, 연도는 date). `npm pack --dry-run`으로 LICENSE 포함 확인 — 자동 포함 안 되면 `packages/ui/LICENSE`로 복사. `.npmignore`에서 `tsup.config.ts` 줄 삭제. 완료 조건: pack 결과에 LICENSE·package.json 포함. 커밋: `chore(ui): npm 배포 메타데이터·LICENSE 추가`
- [x] **P2b** (2026-09-17 `446c660`, chore/ui-publish-meta) ui — `vite.config.ts` `dts({ include: ['src'], exclude: ['**/*.stories.tsx', '**/*.test.tsx'], insertTypesEntry: true })`(지금 dist에 `*.stories.d.ts` 24개·`*.test.d.ts` 24개가 섞인다). 완료 조건: `pnpm --filter galley-ui build && npm pack --dry-run`에 stories/test d.ts 0개, 총 파일 60개 미만. 커밋: `chore(ui): d.ts 출력에서 스토리·테스트 제외`

### P3. 다크 토큰 — 브랜치 `design/ui-dark-tokens`

- [x] **P3a** (2026-09-17 `084dcc1`, design/ui-dark-tokens) ui — `src/tokens/tokens.css` `[data-theme='dark']` 빈 블록에 `--ui-color-*` 25개 재정의(space·text·radius는 그대로, shadow는 다크에서 필요하면만). 기준: 본문 텍스트 대비 4.5:1 이상 · accent는 라이트와 같은 계열에서 밝기만 조정 · topbar는 값 유지 또는 한 단계만. 완료 조건: 대비값 계산표를 worklog에 남김 · decisions/ui-style.md 갱신 이력 "다크 값 확정" 한 줄. 커밋: `design(ui): 다크 테마 색 토큰 값 추가`
- [x] **P3b** (2026-09-17 `ff83974`, design/ui-dark-tokens) ui — `/design` 갤러리에 개발환경 전용 테마 토글(`document.documentElement.dataset.theme`). 앱 테마 저장 기능은 만들지 않는다(범위 밖). 완료 조건: 갤러리 전 섹션을 다크로 육안 확인 · `verify:layout` 통과. 커밋: `feat(dashboard): 갤러리에 개발용 테마 토글 추가`
...
- [x] **P4-1** (2026-09-17 `af19ef9`·갤러리 `88577ff`·리뷰 8건 `a5e0525`–`82b9d82`, feat/ui-separator) ui — **Separator**(components) — `orientation` horizontal|vertical · `decorative`(true면 role none, 아니면 separator) · 토큰 border 색. 브랜치 `feat/ui-separator`. 커밋: `feat(ui): Separator 컴포넌트 추가`
- [x] **P4-2** (2026-09-17 `237dc6a`·갤러리 `69ba57f`·리뷰 5커밋 `7a1bb1e`–`e31037b`, feat/ui-switch) ui — **Switch**(primitives, Base UI Switch) — 제어형 `checked`/`onCheckedChange` · `children` 라벨(없으면 `aria-label` 필수) · `disabled` · `name`. Checkbox와 같은 label 감싸기. 브랜치 `feat/ui-switch`. 커밋: `feat(ui): Switch 프리미티브 추가`
- [x] **P4-3** (2026-09-17 `6991306`·갤러리 `b6e1dea`·리뷰 9커밋 `f7392ab`–`808da63`, feat/ui-radio-group) ui — **RadioGroup**(primitives, Base UI Radio/RadioGroup) — 제어형 `value`/`onValueChange` · `items: {value,label,description?,disabled?}[]` · `aria-label` · `orientation`. 아이템 레이아웃은 ItemContent 재사용. 브랜치 `feat/ui-radio-group`. 커밋: `feat(ui): RadioGroup 프리미티브 추가`
- [x] **P4-4** (2026-09-17 `fedd070`·Textarea 연결 `ff17dd8`·required `155dbdd`·hidden input 기준점 `aaf96ac`·리뷰 커밋 `8c6dfa5`–`27d4ab4`, feat/ui-form-field) ui — **FormField**(primitives, Base UI Field) — `label` · `description?` · `error?`(string, 있으면 aria-invalid·aria-describedby 연결) · `required?` · `children`에 Input/Textarea/Select/Switch. Input·Textarea `invalid`는 FormField error가 있으면 Base UI Field 컨텍스트로 자동(연결 안 되면 소비자가 `invalid`를 직접 넘기는 방식으로 두고 보고). 브랜치 `feat/ui-form-field`. 커밋: `feat(ui): FormField 프리미티브 추가` **메모(P4-3 리뷰, 2026-09-17)**: Base UI Field 안에서는 RadioGroup 그룹에 `aria-labelledby`가 자동 주입된다 → RadioGroup의 `aria-label` 필수를 선택으로 완화할지 여기서 검토.
- [x] **P4-4b** (2026-09-17 `524a1a6`·갤러리 `d1dc8c4`·실측 `6a5f1a6`·리뷰 커밋 `ddb9b43`–`c7948ed`, feat/ui-form) ui — **Form**(primitives, Base UI Form) — 2026-09-17 사용자 결정(P4-4에서 파생). FormField의 `required`·네이티브 검증 문구가 **인라인으로** 뜨려면 폼이 Base UI Form이어야 한다(일반 `<form>`은 브라우저가 submit 전에 막고 말풍선만 띄워 Field 검증이 돌지 않는다 — Chrome 실측). 범위: `onSubmit` · `errors?`(서버 오류를 필드 name으로 전달) · `children` · `className` · `aria-label`. 확인할 것: noValidate로 말풍선이 꺼지는지 · 제출 실패 시 첫 오류 필드로 포커스 · FormField의 `name` 연결 방식(지금은 컨트롤에 name을 준다 — `errors` 매칭은 Field.Root `name` 기준이라 FormField에 `name?` prop이 필요할 수 있다) · 앱이 넘긴 `error`와 `errors`의 우선순위. 갤러리 FormField 데모를 Form으로 바꾸고 `verify-layout/form-field.mjs`의 "빈 채 제출" 검사를 인라인 문구 기준으로 맞춘다. 브랜치 `feat/ui-form`. 커밋: `feat(ui): Form 프리미티브 추가`
- [x] **P4-5** (2026-09-17 `ba51e3e`·갤러리 `81a5c79`·타입 강제 `9615075`·리뷰 커밋 `750e1aa`–`1177aca`, feat/ui-popover) ui — **Popover**(primitives, Base UI Popover) — `trigger` · `children` · `title?` · `side` · `align` · `open`/`onOpenChange`(선택 — 없으면 비제어. **제어형 원칙의 유일한 예외**, 이유를 README 설계 원칙에: 단순 정보 팝업은 앱이 상태를 가질 이유가 없음). Tooltip과 같은 trigger 병합. 브랜치 `feat/ui-popover`. 커밋: `feat(ui): Popover 프리미티브 추가`
- [x] **P4-6** (2026-09-17 `ca0d29b`·갤러리 `9b048f5`·plain variant `dc4f811`·리뷰 커밋 `f1befa6`–`957ea22`, feat/ui-inline-alert) ui — **InlineAlert**(components) — `tone` info|success|warning|danger · `title?` · `children` · `action?` 슬롯 · `role`은 danger·warning이면 alert, 아니면 status. 아이콘 lucide. **TD2의 ui 부분.** 앱 4곳 치환(fe)은 포함하지 않는다 — 완료 시 TD2에 "ui 완료, 앱 치환 남음" 표시. 브랜치 `feat/ui-inline-alert`. 커밋: `feat(ui): InlineAlert 컴포넌트 추가`
- [x] **P4-7** (2026-09-21 `ea96f78`·갤러리 `4e9f972`·실측 `029d668`·리뷰 커밋 `3e15f6a`–`11177b1`·회귀 수정 `068f491`·최저 opacity 0.6 `54ea48d`, feat/ui-skeleton) ui — **Skeleton**(components) — `width`/`height`(토큰 space 또는 CSS 값) · `radius` sm|md|full · `lines?`(텍스트 여러 줄) · `aria-hidden` 기본 · 부모가 `aria-busy` 다는 규칙을 스토리에 예시. 브랜치 `feat/ui-skeleton`. 커밋: `feat(ui): Skeleton 컴포넌트 추가`
- [x] **P4-8** (2026-09-21 `ba47385`·갤러리 `655b994`·실측 `3e32b09`·리뷰 커밋 `0daf4d0`–`59bacd4`, feat/ui-toast) ui — **Toast**(primitives, Base UI Toast) — `ToastProvider`(뷰포트 위치 top-right|bottom-right, 최대 개수) + `useToast()` 훅(`toast({ title, description?, tone?, duration? })`). 훅은 `hooks/`에 두고 배럴 export. 키보드 접근(F6/포커스 이동)·`role=status`·수동 닫기 버튼은 Base UI 기본. Base UI에 Toast가 없으면 멈추고 직접 작성 범위 보고(2026-09-15 확인: 있음). 브랜치 `feat/ui-toast`. 커밋: `feat(ui): Toast 프리미티브와 useToast 훅 추가`
- [x] **P4-9** (2026-09-21 훅 `82df2a1`·Button danger `f89dc1f`·갤러리+실측 `62fe22a`·리뷰 `cb247cb`–`481075b`, feat/ui-await-dialog — **구현은 스펙과 시그니처가 다르다**: `useAwaitDialog<T>(cancelValue)`, 렌더 함수는 `{ open, onOpenChange, resolve, cancel }`; Dialog `initialFocus` prop·토큰 `--ui-color-danger`/`-hover` 추가. worklog 09-21) ui — **useAwaitDialog + useConfirm**(hooks) — Promise로 기다리는 다이얼로그. RunActionBar가 확인 Dialog를 `useState` 두 벌로 드는 패턴을 훅 하나로 대체하는 것이 목적(앱 치환은 후속 P7). 폴더 `hooks/useAwaitDialog/{index.ts, useAwaitDialog.ts, useConfirm.tsx, *.test.tsx, *.stories.tsx}`.
...
- [x] **P4-10** (2026-09-21 `fcf6280`, P5 PR) ui — 9개 완료 후 todo/ui-engineer-todo.md의 "남은 것: Popover" 줄 정리(P5 PR에 포함).
- [x] **P4-11** (2026-09-21 children `c2167a8`·useAlert `7f2025b`·갤러리+실측 `745e988`·치환 `829a0e2`·리뷰 `06fd6d1`–`472aa28`, refactor/dashboard-use-confirm, worklog 09-21) fe — RunActionBar의 확인 Dialog 2개(재실행·승인)를 `useConfirm`으로 치환(P7a에서 앞당김, 2026-09-15 사용자 결정 — 훅 API를 실제 소비자에서 검증한 뒤 배포). `useState` 두 벌·Dialog JSX 제거, 테스트 9건은 동작 유지 확인. 브랜치 `refactor/dashboard-use-confirm`. 커밋: `refactor(dashboard): RunActionBar 확인 Dialog를 useConfirm으로` **결정(2026-09-21, decisions/confirm-dialog-usage.md)**: 버튼 md 통일 · 확인 즉시 닫힘 + pending 동안 `role=status`에 "요청 중" 문구 · `ConfirmOptions.children?` 추가(재실행 계획 표) · Provider 보류 · 축약 호출 없음 · **`useAlert` 이 PR에서 추가**(`hooks/useAwaitDialog/useAlert.tsx`, 갤러리·실측 포함). **P4-9 리뷰 L7 입력(2026-09-21)**: 현재 RunActionBar Dialog와 안 맞는 것 셋 — ① footer 버튼 `size="sm"`(useConfirm은 md) ② `disabled={pending}`으로 액션이 도는 동안 Dialog를 열어 둠(useConfirm은 확인 즉시 닫힘 → 동작 변화, 바 쪽 pending 표시로 대체) ③ 재실행 Dialog의 `children`(계획 `<dl>`) — `description`은 `<p>`라 `<dl>`을 넣으면 잘못된 중첩. 방향: 승인만 useConfirm, 재실행은 `useAwaitDialog<boolean>` 직접(children 유지) 또는 ConfirmOptions에 `children?` 추가 — 착수 때 결정.
...
- [x] **RF1** (2026-09-21, 37항목 — worklog 09-21 RF 절) reviewer — `packages/ui` 전체 감사 목록(코드 수정 없음). 관점 고정: (1) **API 일관성** — 제어형(value+onChange) · className 병합 · 접근성 이름 prop · 이벤트 이름 `onXxxChange` · size/tone/variant 어휘 통일 · props 타입 전부 배럴 export (2) **스타일** — 토큰 위반(하드코딩 색·px) · Input/Select/Textarea 공통 필드 스타일 중복 · 폴더 5파일 규칙 · 스토리 누락 (3) **경계** — 배럴 누락·불필요 export · `'use client'` 표시 일관성 · 죽은 코드·주석 · 도메인 단어 (4) **테스트** — 중복·누락(키보드·포커스) (5) **산출물** — `dist` 크기·external·sideEffects·d.ts 내용. 결과는 worklog 표(항목·심각도·호환 영향·추천)로. 커밋 없음.
  - RF1 입력(P4 리뷰에서 보류해 넘긴 것, 2026-09-17): ① Checkbox·Switch — `children`도 `aria-label`도 없는 이름 없는 컨트롤을 타입이 못 막는다(판별 유니온 후보, 둘을 함께) ② `0.12s ease` 전환 하드코딩이 Button·Switch 두 곳 → duration·easing 토큰화, `prefers-reduced-motion` 대응은 Switch에만 있음(Button에도) ③ 비활성·선택됨 표현이 3종 제각각 — Radio(빈 원 + muted 점) · Switch(muted로 채운 트랙) · Checkbox(border 채움 + surface 체크, 대비 ≈1.2:1로 가장 약함). 통일 여부와 Checkbox 대비. ④ 토글·라디오(Switch·Checkbox·RadioGroup)는 invalid여도 컨트롤 모양이 그대로(P4-4 L3, 현행 유지 결정) — 감사 때 재확인만. ⑤ RadioGroup `aria-label` 필수 완화(`aria-label | aria-labelledby` 유니온)는 ①과 함께. **(P4-7 리뷰, 2026-09-21)** ⑥ Skeleton `radius` 값 `full`이 토큰 `--ui-radius-pill`과 어휘가 다르다 — size/tone/variant 어휘 통일 때 같이 ⑦ ②에 추가: 펄스 duration도 하드코딩(Badge 1.2s·Skeleton 1.5s)이고 Badge 펄스에는 `prefers-reduced-motion` 대응이 없다(Skeleton·Switch에는 있음). **(P4-8 리뷰, 2026-09-21)** ⑧ 층 순서가 숫자로 흩어져 있다 — Toast 뷰포트 `z-index: 2`, Menu·Select·Popover·Tooltip은 1, Dialog는 없음 → `--ui-z-*` 토큰 ⑨ `ToastTone`과 `InlineAlertTone`이 같은 유니온이고 아이콘 4개 매핑도 동일 → 공유 `Tone` 타입·아이콘 매핑 유틸(어휘 통일과 함께) ⑩ 폴더 규칙: `primitives/Toast/Toast.tsx`가 export하는 것은 `ToastProvider`(파일명 = 컴포넌트명 규칙과 어긋남). **(P4-9 리뷰, 2026-09-21)** ⑪ ②에 추가: Button 어느 variant에도 `:focus-visible` 스타일이 없어 UA 기본 링에 의존(danger에서 처음 눈에 띔) ⑫ ⑨에 추가: `ConfirmTone('default'|'danger')`이 Toast·InlineAlert `tone`·Button `variant`와 또 다른 어휘(+ P4-11: useAlert가 useConfirm의 `ConfirmTone`을 역방향 import, Dialog+footer+tone→variant 매핑이 useConfirm·useAlert에 중복 → 폴더 공용 tone 타입 + `toneVariant()` 헬퍼) ⑬ 기본 라벨 한국어 하드코딩(useConfirm '확인'·'취소', Dialog '닫기', Toast '알림') — 공개 npm 관점에서 한 번에(P5a README에 라벨 prop 안내). **(P4-11 리뷰, 2026-09-21)** ⑭ 비동기 onClick 패턴 둘 공존 — 갤러리 `onClick={async () => …}` vs RunActionBar `() => void onX()`(ESLint에 no-floating-promises 없음) → 하나로 통일 + 규칙 검토.
- [x] **RF2** (2026-09-21 — 호환 깨는 정리 **허용**, 추천대로, RF-11만 현행 유지) 사용자 — RF1 항목 승인/기각. **미결(해소): 0.1.0 전이라 호환 깨는 API 정리를 허용할지**(추천: 허용 — 소비자가 dashboard뿐이고 배포 뒤에는 major가 든다. "범위 밖: 기존 API 변경" 규칙의 유일한 예외). 기각·보류 항목은 P7에 적는다.
- [x] **RF3** (2026-09-21 `e8a773c`–`d43eb26` 25커밋, refactor/ui-pre-release) ui(+fe: dashboard 따라가기) — 승인 항목당 커밋 하나(`refactor(ui): …`, dashboard 수정은 `refactor(dashboard): …`). 완료: `pnpm lint && pnpm typecheck && pnpm test` · `pnpm --filter galley-ui build` · `verify:layout` · dashboard `next build` · 갤러리 스크린샷 전후 동일. worklog에 승인/기각 판단 기록(/review 규칙).

### P5. README — 브랜치 `docs/ui-readme` (RF 뒤)

- [x] **P5a** (2026-09-21 `2807fc3`, docs/ui-readme — 163줄, 스펙 절 전부 + RF API 변화·useAlert·모션 절) doc — `packages/ui/README.md` 신규(영어만): 한 줄 소개 → 설치(`pnpm add galley-ui` + peer react/react-dom ^19) → 30초 예제(`import 'galley-ui/styles.css'` + AppShell 안 Button·Badge) → `useConfirm` 예제(await 한 줄, 차별점이라 앞쪽) → 다크 모드(`data-theme="dark"`) → 컴포넌트 표(components/primitives/patterns/hooks 각 한 줄) → 설계 원칙(제어형만(Popover 예외)·Base UI 얇은 래퍼·CSS Modules+토큰·도메인 무지·번들 전체 "use client"·Node ≥20) → **접근성 절**(키보드·포커스·라벨·대비 기준 + "forced-colors(Windows 대비 테마)는 미검증 — Separator만 대응" 명시, 2026-09-17 사용자 결정) → Storybook 예정(Phase 2) → 라이선스. 커밋: `docs(ui): galley-ui README 작성` **Form·FormField 절에 넣을 것(P4-4·P4-4b 리뷰, 2026-09-17)**: ① `required`는 FormField에만 적는다 — 안쪽 컨트롤이 native required를 물려받는다 ② 일반 `<form>`에서는 브라우저 말풍선, `Form` 안에서만 오류가 필드 아래에 인라인 ③ `errors` 키는 컨트롤 name · **참조를 안정적으로(state·상수)** — 인라인 객체면 제어형 입력에서 오류가 되살아나 제출이 막힌다 ④ 오류가 남은 필드는 제출을 막는다 — 앱 `error`는 앱이 지우고, 제출 뒤 정해지는 오류는 `errors`로 ⑤ `onFormSubmit`이 없으면 `action`이 그대로 제출 · 이벤트 정보는 native `onSubmit` ⑥ 제출 뒤부터는 change마다 재검증 ⑦ 기본 세로 배치는 `:where` 명시도 0 ⑧ FormField 안 Switch·Checkbox는 children 없이 ⑨ `Form<Values>` 제네릭은 타입만(런타임 검사 없음). **Popover(P4-5, 2026-09-17)**: 설계 원칙 절에 "제어형만"의 유일한 예외와 이유(단순 정보 팝업은 앱이 열림 상태를 가질 이유가 없다) · `open`을 주면 `onOpenChange`가 타입으로 필수 · `onOpenChange(false)`는 포커스 이탈로도 온다 · 제목은 heading이 아니다(비모달). **InlineAlert(P4-6, 2026-09-17)**: live 영역은 제목+본문만(action은 밖) · alert tone은 조건부 렌더로 읽히지만 status tone은 보장되지 않는다(필요하면 앱의 상시 live 영역 안에) · 정적 안내에는 info · `variant="plain"`의 쓰임. **Toast(P4-8, 2026-09-21)**: `ToastProvider`는 앱 루트에 한 번, 상단 바가 있으면 `:root`에서 `--ui-toast-inset-top`을 `calc(바 높이 + var(--ui-toast-inset))`로 · 닫기 버튼은 뷰포트가 펼쳐지기(hover·포커스) 전까지 보조 기술에 숨겨진다(Base UI — Esc·F6이 경로) · danger·warning은 `alertdialog`로 끼어들어 읽히고 포커스 전까지 aria-hidden · `useToast()`는 목록을 구독하지 않아 리렌더 없음, 마운트 effect에서 띄워도 됨 · 모달 Dialog가 열린 동안 띄운 토스트가 보조 기술에 읽히는지는 미검증(주의 문구). **useAwaitDialog·useConfirm(P4-9, 2026-09-21)**: `useAwaitDialog<T>(cancelValue)` — 닫힘은 언제나 resolve(Esc·바깥 클릭·닫기·`cancel()` = cancel 값, reject 없음) · 렌더 함수는 렌더 중에 불리므로 훅 금지, 상태가 필요하면 컴포넌트를 반환(새 open마다 재마운트되어 이전 입력이 남지 않음) · 클로저는 open() 시점 고정 · 재open은 이전 Promise를 cancel로 · 자식 마운트 effect에서 열면 StrictMode(dev)에서 첫 Promise가 cancel · `useConfirm` 초기 포커스는 danger=취소·기본=확인, `tone: 'danger'`는 Button danger · Dialog `initialFocus` prop. **RF(2026-09-21) README에 반영할 API 변화**: 기본 라벨은 영어(Dialog `closeLabel` 'Close' · ToastProvider `aria-label` 'Notifications'·`closeLabel` 'Close' · useConfirm 'Confirm'/'Cancel' · useAlert 'OK') — 한국어 앱은 호출부에서 넘긴다 · Badge는 `tone`(색), `variant`는 모양·역할(Button·InlineAlert) · ToastProvider·ActionBar 접근성 이름은 `aria-label` · Skeleton radius `sm|md|lg|pill` · 타입 export 목록(BadgeTone·ButtonVariant·ButtonSize·SeparatorOrientation·StatTileTone·RadioGroupOrientation·MenuAlign) · 이름 없는 Checkbox·Switch·RadioGroup은 개발 모드 경고(FormField 안은 예외) · 모션·포커스 링·z 토큰(`--ui-duration-*`·`--ui-focus-ring`·`--ui-z-*`)과 reduced-motion 자동 대응 · SidebarItem·ListToolbarTab `href`. **useAlert(P4-11)**: 확인 하나·`Promise<void>`·어떻게 닫아도 같음·`role=dialog`(alertdialog 아님, 끼어들기 낭독 없음) · `ConfirmOptions.children`은 블록 본문용(description은 `<p>`) · 확인 뒤 비동기 작업의 진행 표시는 소비자 몫(RunActionBar 예: busy 상태 + `role=status`).
- [x] **P5b** (2026-09-21 `ff82673`) doc — 루트 README.md에 "packages/ui는 npm `galley-ui`로 배포" 한 줄 + 링크. 커밋: `docs(docs): 루트 README에 galley-ui 배포 안내`
...
- [ ] **P7c** — 문서(소개) 사이트 → GitHub Pages. 2026-09-21 사용자 결정: 0.1.0은 README만, 사이트는 추후. 선택지는 그때 `/decide`: (a) Storybook 정적 빌드(스토리 30개 기존, 설치·preview 토큰 CSS·다크 툴바·a11y 애드온·Actions 배포, 반나절) (b) `apps/docs` Next `output: 'export'` 랜딩 페이지(`basePath: '/galley'`, 하루 이상, 컴포넌트 카탈로그는 별도) (c) 둘 다. 사이트가 생기면 README에 링크 추가(patch 버전).
- [ ] **P7d** ui — Table · Avatar · Breadcrumb · Pagination.
- [ ] **P7e** fe — TD2 앱 자리 InlineAlert 치환. 실제 자리는 **5곳**(2026-09-17 P4-6 리뷰에서 정정): `queue/page.tsx`·`DraggableQueueRows` → filled 그대로 / `QueueRowMenu`(목록 행 안)·`MissingTopicsNotice`(이미 상자 안) → `variant="plain"` / `ReloadQueueButton`은 상시 `role="status"` 한 곳에서 성공·실패 문구를 바꿔 끼우는 구조라 **현행 유지**(InlineAlert는 tone이 role을 정하고 조건부 렌더 전제). 치환 뒤 각 파일의 `.error` CSS 삭제.
...
- [ ] **P7h** 사용자 — VoiceOver+Chrome으로 Form 제출 실패 낭독 실측(오류 2개 이상인 폼). FormField의 상시 `aria-live="polite"` 영역 N건 + 첫 오류 포커스의 `aria-describedby` 낭독이 겹쳐 장황한지 확인. 겹치면 선택지: 허용(정보 손실 없음) · Form 안에서는 live를 끄기 · 제출 요약 분리. P4-4b 리뷰 M2에서 보류(2026-09-17). **같이 볼 것(P4-5 L6)**: 본문이 글자뿐인 Popover를 열면 포커스가 팝업 자체로 가서 "제목, dialog"만 읽히고 본문은 자동 낭독되지 않는다 — 불편하면 `description?` prop(aria-describedby) 추가를 검토. **같이 볼 것(P4-7 M2, 2026-09-21)**: Skeleton 갤러리의 "aria-busy 부모 + 그 밖 상시 role=status 텍스트" 조합이 VoiceOver에서 실제로 "불러오는 중"을 읽는지, aria-busy 안 변화가 보류되는지 확인. **(P4-11 리뷰)** RunActionBar `role=status` 오류/진행 문구 낭독 · 확인 뒤 포커스 복귀(Chrome: 승인 → Enter → 실패 응답 뒤 activeElement가 승인 버튼인지 — happy-dom 재현 불가).
- [ ] **P7i** ui — Dialog 안에서 Popover를 열 때 실측: 바깥 클릭이 Dialog까지 닫지 않는지 · Esc 한 번에 Popover만 닫히는지(같은 구조의 Select-in-Dialog는 동작 중). 갤러리에 케이스가 없어 P4-5 리뷰 L7에서 보류(2026-09-17).
...
- [ ] **TD8** pl — 이전 편 제목·시리즈명·주제 제목이 프롬프트·발행 본문으로 가는 경로에 식별 정보 필터(`.galley/redact.json`)를 걸지 결정 — 지금은 `stripTopicHints`(괄호 힌트)만. 닫히지 않은 괄호(`제목 (spacehome`)는 별칭이 그대로 나간다(BX3 리뷰 L8, 사용자 "기록만"). 커밋: `fix(pipeline): 제목 경로에 식별 정보 필터 적용`
- [ ] **TD7** pl — **확인 필요**: 파일에서 사라져 자동 보류된 행(`holdReason=removed-from-file`)이 매 적재마다 다시 매칭되지 않아 update되는 것으로 보인다 → `updatedAt`이 계속 바뀌어 자동 연결이 매 틱 재계산될 수 있다(BX2 리뷰 14, 이번 브랜치 밖). 재현 테스트 먼저, 맞으면 이미 보류된 행은 건너뛴다. 커밋: `fix(queue): 자동 보류된 행을 매 적재 갱신하지 않기`
...
- [ ] **TD2** ui+fe — **ui 완료(2026-09-17 P4-6, `InlineAlert` filled·plain + 갤러리 + verify:layout 12검사), 앱 치환 남음(P7e)**. 실패 표시가 앱에 흩어져 있다(실제 5곳 — P7e 참고): `role="alert"` + 각 파일의 `.error` CSS가 4곳(큐 화면·행 ⋮ 메뉴·드래그 목록·다시 불러오기 버튼)에 복제. `galley-ui`에 표현 전용 컴포넌트(예: `InlineAlert` — 도메인 단어 없이 tone·children만)로 모으고 앱은 문구만 넘긴다. **ui 추가이므로 갤러리 섹션 + `verify:layout`까지가 완료 조건.** decisions/error-handling.md 미결. 커밋: `feat(ui): InlineAlert 컴포넌트 추가`
