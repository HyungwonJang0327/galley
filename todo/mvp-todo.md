...
- [x] **BE5** (2026-09-22 `c9374cb`·`e848060`·`e2062eb`·`c44b251`·`f65487c`·`3a99bbb`·리뷰 수정 `d77b152`·`27dd1bc`·`9aefde5`·`5e90799`·`3f9b996`·decisions `2d6b0d0`·`79a0489`, feat/index-job-runner — 한 틱 = 배치 하나(Run 우선), 작업은 클레임 시점 `toSha` 고정, IndexJob 쓰기는 소유 조건, CLI는 등록+큐잉만) pl — 증분 재인덱싱(HEAD 비교 → `stale`, 바뀐 경로·새 커밋 범위만) + `--full` + CLI `bin/index.ts`(`pnpm --filter @galley/pipeline index <path> [--name --alias --read-only --model --full]`, 모델 기본 = 레지스트리 `indexingDefault` Haiku 4.5, 사용 모델을 `Repo.lastIndexModelId`에 기록). 커밋: `feat(pipeline): 증분 재인덱싱과 index CLI 추가`
