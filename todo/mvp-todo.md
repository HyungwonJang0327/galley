...
- [x] **P4-5** (2026-09-17 `2747a0d`·갤러리 `14a3ec0`·타입 강제 `1fce1a6`·리뷰 커밋 `c739136`–`757494d`, feat/ui-popover) ui — **Popover**(primitives, Base UI Popover) — `trigger` · `children` · `title?` · `side` · `align` · `open`/`onOpenChange`(선택 — 없으면 비제어. **제어형 원칙의 유일한 예외**, 이유를 README 설계 원칙에: 단순 정보 팝업은 앱이 상태를 가질 이유가 없음). Tooltip과 같은 trigger 병합. 브랜치 `feat/ui-popover`. 커밋: `feat(ui): Popover 프리미티브 추가`
