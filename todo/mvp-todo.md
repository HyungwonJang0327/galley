...
- [x] **BX1** (2026-09-24 브랜치 `7df7c3a`·`bb6b530`·`bf06439`·`110b73d` + 리뷰 `0c08456`·`cfc0ccc`·`5588f57` — main 해시는 머지 후 정정, feat/queue-series-parse) pl — 큐 파서: `QueueTopic.series?: { key, episode }`(태그 뗀 `title`), 라이터가 태그를 다시 붙임(완료 줄은 `날짜 [A-1] 제목`), `ParsedQueue.seriesDefs`(후보 `### 시리즈` 아래 정의 줄, **라이터가 같은 자리에 되돌림** — 지금은 되쓰기 한 번에 사라진다), `moveTopic`이 `series`를 유지, `stripTopicHints`·`normalizeTopicTitle`이 태그를 먼저 뗌. 픽스처 `__fixtures__/series.md`(~~실제 파일 복사~~ → **합성**: 실제 파일 복사가 auto mode에서 거부돼 사용자 결정 — 구조만 동일, 정의 7·태그 18, `.prettierignore`). 브랜치 `feat/queue-series-parse`. 커밋: `feat(queue): 시리즈 태그 파싱과 정의 줄 보존` / `test(queue): 시리즈 fixture 라운드트립` / `feat(queue): 매칭 키에서 시리즈 태그 제외`
...
- [ ] **BX2** pl — DB 적재: `QueueItem.seriesKey String?`·`episodeNo Int?`(마이그레이션 1개, `@@index([seriesKey, episodeNo])`), `parsedQueueToRows`가 채움(재적재 시 재생성, 정의 줄은 DB에 안 넣음), `isUnchanged` 비교에 포함. `getSeriesContext(seriesKey)` → `{ key, name, nameJa?, episodes: [{ episodeNo, title, status, slug?, velogUrl?, alreadyPublished }] }`(이름은 파일 seriesDefs, 편은 DB 세 상태 합침, 슬러그는 완료 줄 `(posts/<슬러그>)`, velogUrl은 완료 줄의 `https://velog.io/...`, `(기존 글)`은 alreadyPublished). 브랜치 `feat/queue-series-columns`. 커밋: `feat(db): QueueItem에 seriesKey·episodeNo 추가` / `feat(queue): 시리즈 컨텍스트 조회 헬퍼`
  - BX1 이월(리뷰): 완료 줄 URL을 `QueueTopic` 필드로 분리(L3 — 지금은 title에 남고 키에서만 뗌) · 사라진 줄 복원 시 `seriesKey`·`episodeNo`로 태그 재부착(M3) · `SeriesTag`·`SeriesDef` 배럴 export(L5) · 정의 줄 원문 보존 여부(L1 — 이동 한 번에 전각 괄호·공백이 정규화됨, `ja:` 추출 때 같이 판단).
