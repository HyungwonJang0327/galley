...
- [x] **A6a** (2026-09-12 `2ff4541`·`6ac03d4`·`2c4098d`·`7e06d14`, feat/queue-reorder-dnd) fe — pragmatic-drag-and-drop 도입 + 대기 큐 순서 변경 UI. 사용자 결정: 대기 탭 안에서만 · 놓자마자 저장 · 키보드 이동은 범위 밖. **후보는 `UNSUPPORTED_SECTION`으로 거부**(### 소제목 때문에 카테고리가 생김). 드래그 조작 자동 검증은 불가(합성 DragEvent는 `isTrusted: false`) → 수동 확인(사용자 결정). 커밋: `feat(dashboard): 큐 순서 변경 DnD 추가`
