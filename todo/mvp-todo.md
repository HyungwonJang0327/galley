...
- [ ] **P7h** 사용자 — VoiceOver+Chrome으로 Form 제출 실패 낭독 실측(오류 2개 이상인 폼). FormField의 상시 `aria-live="polite"` 영역 N건 + 첫 오류 포커스의 `aria-describedby` 낭독이 겹쳐 장황한지 확인. 겹치면 선택지: 허용(정보 손실 없음) · Form 안에서는 live를 끄기 · 제출 요약 분리. P4-4b 리뷰 M2에서 보류(2026-09-17). **같이 볼 것(P4-5 L6)**: 본문이 글자뿐인 Popover를 열면 포커스가 팝업 자체로 가서 "제목, dialog"만 읽히고 본문은 자동 낭독되지 않는다 — 불편하면 `description?` prop(aria-describedby) 추가를 검토.
