...
상태 머신: `실행 → 승인 대기 → (수정 지시 → 시작 단계부터 재실행) → 완료`. 근거 검증의 unsupported는 표시 조건이지 실패가 아니다. 재실행은 **시작 단계 + 이후 전부**(Select 지정 우선, 없으면 지시에 "근거·커밋·코드"가 있으면 근거 수집부터·없으면 본문부터). 범위에 없던 앞 단계는 `carried`(이전 결과 유지) — decisions/evidence-collection.md.
**시리즈**는 `주제_큐.md`의 표기로만 존재한다(편 줄 태그 `[A-1]`, 후보 `### 시리즈` 아래 정의 줄 `시리즈 A. <이름> (<메모>)`). DB는 `seriesKey`·`episodeNo` 캐시뿐, 이름·편 목록은 매번 파일에서. 태그는 표시 제목·매칭 키·슬러그에 안 들어가고 화면은 배지로 보인다. 발행 제목 `| 시리즈명 N편`·Zenn `(第N回)`, `(기존 글)` 편은 실행 대상이 아니다 — decisions/series.md.
...
│     │  │  ├─ runs/           # 실행 상세(2분할)·이력 (패턴 B) — 타임라인 6줄(펼침: 마크다운 미리보기·썸네일·근거 항목·검증 주장 목록)
...
- **에러 처리**: 예상된 실패는 **값으로** 돌려준다(던지지 않는다). 층마다 형태가 다르다 — 도메인(`packages/pipeline`)은 `{ ok:false, code }`(문구 없음), 앱 어댑터(`apps/dashboard/lib`)가 코드→한국어 문구를 붙여 `{ ok:false, error:{ code, message } }`, Route Handler는 거기에 HTTP 상태를 매핑(성공은 `{ ok:true, data }`). `throw`는 프로그래머 오류·외부 SDK 경계·도달 불가 분기에만. 자세히는 decisions/error-handling.md.
...
- 레이아웃 컴포넌트(AppShell·Sidebar·TopBar·ListRow 등)를 건드리거나 갤러리(`/design`)를 바꾸면 `pnpm --filter dashboard verify:layout` 통과가 완료 조건(decisions/layout-measurement.md). 스크립트는 갤러리 aria-label에 결합되어 있으니 갤러리를 바꾸면 스크립트도 맞춘다. CI `layout` 잡이 PR마다 같은 스크립트를 돌린다(required check — 실패 시 머지 불가, 스크린샷 artifact).
