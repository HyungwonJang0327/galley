...
- **사이드바 IA(메뉴 이름·구성·순서) 재정비 — 유저 흐름 기반.** (2026-09-08 브레인스토밍 착수, 세션 재개 예정.) 현재 IA는 도메인 명사 그룹(주제·실행·발행·설정) + 상태 하위. 사용자가 "지금 순서·구성이 안 와닿는다 → 유저 사용 흐름대로 재정비" 요청. **아키텍처성**(decisions/layout.md §3 고정 결정 변경).
  - 이미 정해진 것: Content 패턴 2개(**A 목록형** / **B 2분할 상세**)와 화면→패턴 매핑은 고정(layout.md §4). 발행(Zenn 下書き·벨로그 대기)·설정 3화면 **내부 구성은 Phase 2 미확정**. TopBar 워크스페이스 탭(`글`/`설정`)과 사이드바 그룹이 **2층 구조 → '설정' 중복**(재정비 대상).
  - **미결 1차 질문**: 최상위 묶음의 1차 축을 (a) 작업 흐름 단계 순서(큐→실행/검수→발행) / (b) 객체별 유지 + 이름·순서·중복만 정리 / (c) 흐름 기준 초안을 먼저 그려 보고 판단 — 중 무엇으로 둘지. **사용자 선택 대기.**
  - 재개 시: a/b/c 선택 → 구체안 제시 → 확정 시 decisions/layout.md §3 + apps/dashboard/lib/navigation.ts 갱신. **A5c(큐 목록 화면) 착수 전 확정 권장.**
