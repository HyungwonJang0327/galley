...
// 갤러리(/design) Skeleton 섹션의 data-demo("skeleton-shapes"·"skeleton-lines"·"skeleton-card")에 결합되어 있다 —
// 갤러리를 바꾸면 여기도 맞춘다. 전제: shapes는 [sm 120×16, md 120×40, full 40×40] 순서, lines는 lines={3} 하나,
// card는 aria-busy 부모 안에 원 아바타 + 제목(40%) + lines={2}.
...
function measureExpression() {
  return `(async () => {
    ...
    if (!shapes || !lines || !card) return JSON.stringify({ error: 'Skeleton 데모를 찾지 못함' });
