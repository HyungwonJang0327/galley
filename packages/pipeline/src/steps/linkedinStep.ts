...
- 본문의 코드·식별자·경로 중 이미 일반 이름으로 바뀐 것은 그대로 두고, 회사 고유 이름(회사명·도메인·내부 경로·필드명)이
  남아 있으면 어투 규칙대로 일반 이름으로 바꿉니다. 바꾸는 것은 이름뿐이고 새 사실·새 코드는 만들지 않습니다.
- 본문의 대괄호 치환 표기(예: [COMPANY], [EMAIL] 같은 대문자 토큰)는 풀어 쓰지 말고 그대로 둡니다. 어투가 요구하는
  벨로그 링크 자리도 토큰([벨로그 링크]) 그대로 둡니다(발행 뒤 사람이 채웁니다).
