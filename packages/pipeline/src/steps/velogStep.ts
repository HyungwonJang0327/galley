...
export function tonePromptFailure(failure: TonePromptFailure): StepFailure {
  ...
  const message =
    failure.code === 'PROMPT_NOT_FOUND'
      ? `어투 프롬프트 파일이 없습니다(.galley/prompts/${file}).`
      : failure.code === 'PROMPT_EMPTY'
        ? `어투 프롬프트 파일이 비어 있습니다(.galley/prompts/${file}).`
        : `어투 프롬프트 파일을 읽지 못했습니다(.galley/prompts/${file}).`;
 ...
 * 영구. SDK 오류는 `status`를 갖는다. 문구는 고정(원본은 cause로 — 응답 원문·키가 행에 남지 않게).
 */
export function classifyModelError(error: unknown, code: string): StepFailure {
  ...
  const transient =
    ...
    /connection|timeout|ECONNRESET|ETIMEDOUT|EAI_AGAIN/i.test(`${name} ${message}`);
  const refusal = /거부|refus/i.test(message);
...
const SYSTEM_PREFIX = `당신은 아래 "어투" 규칙대로 기술 블로그 초안을 쓰는 작성자입니다. 사실은 오직 사용자 메시지의 "근거 묶음"에 있는 것만 씁니다 —
근거에 없는 수치·경로·함수 이름·에피소드를 지어내지 말고, 확실하지 않은 것은 <!-- [확인 필요] ... --> 주석으로 남깁니다.
대괄호 치환 표기([...])는 풀어 쓰지 말고 그대로 둡니다. 출력은 마크다운 본문 하나이며 앞뒤에 설명·코드 펜스를 붙이지 않습니다.
...
export function buildVelogPrompt(
  ...
  const { bundle } = input;
  const lines: string[] = [`# 주제`, input.topic.title, ''];
    ...
    async run(ctx: StepContext): Promise<StepResult> {
      ...
      if (!read.ok)
        throw new StepFailure(
          ...
          '근거 묶음을 찾을 수 없거나 형식이 깨졌습니다. 근거 수집 단계부터 다시 실행하세요.',
          false,
        );
      ...
      try {
        generated = await adapter.generate({
          system,
          prompt,
          maxOutputTokens: limits.velogMaxOutputTokens,
        });
      } catch (error) {
