...
- **시리즈** — `주제_큐.md`의 표기만으로 연재를 다룬다. 큐 화면에 편 배지(`A-1`)와 시리즈 그룹, 본문에 "○○ 시리즈 N편 · 이전 편 링크", 발행 제목에 `| 시리즈명 N편`. 별도 테이블·편집 UI 없음.

  <!-- prettier-ignore -->
  ```markdown
  ## 대기
  - [A-2] 인증 붙이기 — 웹훅이 첫 요청보다 늦게 올 때        ← 편 줄: 맨 앞 태그
  ## 후보
  ### 시리즈
  시리즈 A. 노트 앱 만들기 (2026.06~07, ja: ノートアプリ)     ← 정의 줄: 이름과 메모(ja:는 Zenn 표기)
  - [A-3] 에러 코드 카탈로그 → 프론트 i18n 매핑
  - [A-5] (기존 글) 세션 방식 다시 보기 https://velog.io/@me/session   ← 이미 발행된 편
  ## 완료
  - 2026-09-20 [A-1] 데이터 모델 (posts/feed-model) https://velog.io/@me/feed-model
  ```
